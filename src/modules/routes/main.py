...
def create_chat():
    ...
    user_id = request.form['user_id']
    cursor.execute("UPDATE \"Message\" SET 'del_mes' = '1' WHERE 'id_message' = " + id_mess + ";")
...
def get_messages():
    user_id = request.form['user_id']
    cursor.execute("SELECT * FOR \"Message\" WHERE 'id_user' = " + user_id + ";")
    mes = ''
    for table in cursor.fetchall():
        mes += table.__str__() + "<br>"
    return mes
...
def get_chats():
user_id = request.form['user_id']
    cursor.execute("SELECT * FOR \"Chat\" WHERE 'id_user' = " + user_id + ";")
    mes = ''
    for table in cursor.fetchall():
        mes += table.__str__() + "<br>"
    return mes
